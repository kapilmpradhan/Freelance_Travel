...
class RedeemerService
{
    public static function addNewRedeemer($userId, $redeemerData, ItemType $itemType)
    {
        try {
            ...
            Logger::error('Failed to add new redeemer', $e, data: [
                'log_file' => config('logging.log_files.errors'),
                'user_id' => $userId,
                'action' => 'add_redeemer_service_failed',
            ]);
            throw new ServiceException(
                message: 'Failed to add redeemer: ' . $e->getMessage()
            );
        }
    }
    ...
    public static function addOrUpdatePrimaryRedeemer($user)
    {
        try {
            ...
            Logger::error('Failed to update primary redeemer', $e, data: [
                'log_file' => config('logging.log_files.errors'),
                'user_id' => $user->uuid,
                'action' => 'update_primary_redeemer_failed',
            ]);
            throw new ServiceException(
                message: 'Failed to update primary redeemer: ' . $e->getMessage()
            );
        }
    }
    ...
    public static function listActiveRedeemers(string|null $userId, ItemType $itemType)
    {
        try {
            ...
            Logger::error('Failed to list active redeemers', $e, data: [
                'log_file' => config('logging.log_files.errors'),
                'user_id' => $userId,
                'action' => 'list_redeemers_service_failed',
            ]);
            throw new ServiceException(
                message: 'Failed to list redeemers: ' . $e->getMessage()
            );
        }
    }
    ...
    public static function removeRedeemer(string|null $userId, string|null $sessionId, $redeemerId)
    {
        try {
            ...
            Logger::error('Failed to remove redeemer', $e, data: [
                ...
                'action' => 'remove_redeemer_service_failed',
            ]);
            throw new ServiceException(
                message: 'Failed to remove redeemer: ' . $e->getMessage()
            );
        }
    }
}
